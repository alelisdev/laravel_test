...
trait FiversTrait
{
    ...
    private static function PrepareTransactions($wallet, $userCode, $txnId, $betMoney, $winMoney, $gameCode, $providerCode)
    {
        ...
        if(!empty($user)) {
            ...
            if (floatval($winMoney) < $bet  && floatval($winMoney) > 0|| floatval($winMoney) === $bet)
            {

                \Log::info("Valor de vitoria menor que aposta ou igual a aposta");
                $typeAction = 'win';
                $transaction = self::CreateTransactions($userCode, time(), $txnId, $typeAction, $changeBonus, $bet, 'fivers', $gameCode, $gameCode);
            ...
            Helper::generateGameHistory($user->id, 'loss', $winMoney, $bet, $changeBonus, $checkTransaction->transaction_id);
