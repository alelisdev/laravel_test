...
trait VeniXTrait
{
     ...
     * @var string
     */
    public static $apiUri = 'https://viper.casino/api/v1/';

    public static function VeniXAuthentication()
    {
        $gateway = GamesKey::first();
        if(!empty($gateway)) {
            $basicToken = base64_encode($gateway->venix_agent_token.':'.$gateway->venix_agent_secret);
        ...
        if($token = self::VeniXAuthentication()) {
            ...
            if(!empty($gateway)) {
                $request = \Illuminate\Support\Facades\Http::withToken($token)
                    ->withQueryParameters([
                        'agent_code' => $gateway->venix_agent_code,
                        'game_id' => $game->game_id,
                        'type' => 'CHARGED',
                        'currency' => 'BRL',
                        'lang' => 'BR',
        ...
        if(!empty($user) && !empty($wallet)) {
            ...
            if($request->type == 'win') {
                ...
                if(!empty($checkTransaction)) {
                    ...
                    \Log::info('4 - '. $checkTransaction->transaction_id);
                ...
                if($request->type == 'bet') {
                    ...
                    \Log::info('1 - '. $request->transaction_id);
